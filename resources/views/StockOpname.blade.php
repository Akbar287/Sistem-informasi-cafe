            ...
            <div class="tile mb-4">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="page-header">
                            ...
                            <a href="{{ url('/stock/opname/create') }}" class="btn btn-primary float-right" title="Tambah Stok"><i class="fa fa-plus"></i> Tambah {{ $title }}</a>
                        </div>
                    </div>
                </div>
                <form action="{{ url('/stock/opname') }}" method="get">@csrf
                ...
                <div class="table-responsive">
                    <table class="table table-hover" id="stock-opname-table">
                        ...
                        <tbody>
                            @if(!is_null($stockOpname) && !empty($stockOpname)) @foreach($stockOpname as $stock)
                            <tr>
                                <td>{{ $stock['code_id'] }}</td>
                                <td>{{ $stock['outlet'] }}</td>
                                <td>{{ $stock['product'] }}</td>
                                <td>{{ ($stock['created_at']) }}</td>
                                <td><a href="{{ url('/stock/opname/' . $stock['stock_opname_id']) }}" class="btn btn-sm btn-primary"><i class="fa fa-info"></i></a></td>
                            </tr>
                            @endforeach
                            @endif
                        </tbody>
