            ...
            <div class="tile mb-4">
                <div class="row">
                    <div class="col-lg-12">
                        <div class="page-header">
                            ...
                            <a href="{{ url('/stock/in/create') }}" class="btn btn-primary float-right" title="Tambah Stok"><i class="fa fa-plus"></i> Tambah Stok Masuk</a>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover" id="stock-table">
                        ...
                        <tbody>
                            @if(!is_null($stockIn) && !empty($stockIn)) @foreach($stockIn as $stock)
                            <tr>
                                <td>{{ $stock->code_id }}</td>
                                <td>{{ $stock->name }}</td>
                                <td>{{ date_format($stock->created_at, "d/m/Y H:i") }}</td>
                                <td><a href="{{ url('/stock/in/' . $stock->stock_card_id) }}" class="btn btn-sm btn-primary"><i class="fa fa-info"></i></a></td>
                            </tr>
                            @endforeach
                            @else
