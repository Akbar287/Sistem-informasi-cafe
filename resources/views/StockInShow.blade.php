...
<div class="app-title">
    <div>
        <h1><a href="{{ url('/stock/in') }}" class="btn"><i class="fa fa-arrow-left"></i></a> {{ $title }}</h1>
    </div>
    <ul class="app-breadcrumb breadcrumb">
        <li class="breadcrumb-item"><a href="{{ url('/home') }}">Home</a></li>
        <li class="breadcrumb-item"><a href="{{ url('/stock/in') }}">Stok Masuk</a></li>
            ...
            <div class="tile mb-4">
                ...
                <div class="row mb-3 justify-content-center">
                    <div class="col-12 col-sm-12 col-md-12 text-center">
                        <div class="btn-group">
                            <a href="{{ url('stock/in') }}" class="btn btn-info" title="Kembali"><i class="fa fa-arrow-left"></i> Kembali</a>
                            {{-- <button type="submit" class="btn btn-primary" title="Tambah Stok"><i class="fa fa-edit"></i> Ubah Stok Masuk</button> --}}
                            <button type="button" class="btn btn-danger" title="Hapus Stok" onclick="event.preventDefault();(confirm('Data Stok Masuk akan dihapus!\nSetelah dihapus data tidak dapat dikembalikan. \nLanjutkan? ') ? document.getElementById('deleteStockIn').submit() : event.preventDefault() );"><i class="fa fa-trash"></i> Hapus Stok Masuk</button>
                            <form action="{{ url('/stock/in/' . $stock->stock_card_id) }}" id="deleteStockIn" method="POST">@csrf @method('delete')</form>
                        </div>
                    </div>
                </div>
            </div>
